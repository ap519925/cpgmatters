 ...
 * Run inside DDEV: ddev exec php setup_article_fields.php
 */
...
foreach ($fields_to_add as $field_name => $info) {
    ...
    // Attach to article bundle if not already
...
//    (so fields show up in the dashboard editor)
// =============================================
...
// Add our new fields to the form
...
// Keep body and field_image, add our new fields
...
// =============================================
// 4. UPDATE THE EXISTING SAMPLE ARTICLE 
//    WITH THE NEW FIELDS
// =============================================
...
if ($node) {
    $node->set('field_summary', [
        'value' => 'Leading consumer packaged goods companies are implementing artificial intelligence and machine learning to optimize inventory management, predict consumer demand, and streamline distribution networks across the globe.',
        'format' => 'basic_html',
    ]);
    $node->set('field_author_name', 'Sarah Mitchell');
    $node->set('field_read_time', '5 min read');
    $node->set('field_image_caption', 'Advanced AI systems are transforming how CPG companies manage their supply chains. Photo: Getty Images');
    $node->save();
    echo "\nUpdated sample article with new field values!\n";
} else {
    echo "\nSample article not found, skipping update.\n";
}

// Also update a couple other articles
$other_articles = [
...
foreach ($other_articles as $title => $fields) {
    $nodes = \Drupal::entityTypeManager()->getStorage('node')->loadByProperties([
        'type' => 'article',
        'title' => $title,
    ]);
